...
func AddRoutes(router *gin.Engine, services *Services) *gin.Engine {
	...
	financeSpace.Use(finances.RequireRole("owner")).DELETE("", financesHandler.DeleteSpace)
	financeSpace.Use(finances.RequireRole("owner")).POST("/members", financesHandler.InviteToSpace)
	financeSpace.Use(finances.RequireRole("owner")).DELETE("/members/:userID", financesHandler.DeleteSpaceMember)
	financeSpace.Use(finances.RequireRole("owner", "editor")).POST("/incomes", financesHandler.CreateIncomeSource)
	financeSpace.Use(finances.RequireRole("owner", "editor")).DELETE("/incomes/:incomeSourceID", financesHandler.DeleteIncome)
	financeSpace.Use(finances.RequireRole("owner", "editor")).POST("/goals", financesHandler.CreateGoal)
	financeSpace.Use(finances.RequireRole("owner", "editor")).POST("/goals/:goalID/contributions", financesHandler.CreateGoalContribution)
